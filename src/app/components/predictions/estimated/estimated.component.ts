...
import {
  PredictionForm,
  PredictionResults,
} from 'src/app/models/predictions.model';
...
export class EstimatedComponent implements OnInit {
  ...
  form: FormGroup;
  windowControl = new FormControl(2, [
    Validators.required,
    Validators.min(2),
    Validators.max(10),
  ]);
  testControl = new FormControl(20, [
    Validators.required,
    Validators.min(1),
    Validators.max(100),
  ]);
  ...
  async makePrediction() {
    ...
    this.results = await this._predictions.projectionByStats(request)
  }
}
