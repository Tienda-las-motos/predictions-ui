...
export class TablesDrawerComponent implements OnInit {
    ...
    if (this.selected !== tableId) {
      ...
      this.tables_.getTable(tableId).subscribe((result: TableData) => {
        ...
        this._alerts.sendMessageAlert(
          `Se cargaron ${result.total_count} de la tabla ${result.file_name}`
        );
      });
    }
  }
  ...
  openUploadBox() {
    var dialog = this._dialog.open(UploadTableComponent, {
      width: '50%',
      maxHeight: '90%',
    });
    ...
    dialog.afterClosed().subscribe((result: TableData) => {
      if ( result ) {
        this._alerts.sendMessageAlert(`
        Se extrajeron ${result.total_count}  resultados del archivo ${result.file_name}
        `);
        this._router.navigate(['/dashboard/table', result.doc_id])
      }
    });
  }
}
